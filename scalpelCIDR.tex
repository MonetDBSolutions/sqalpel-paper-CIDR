...
and which is certainly not \pdfcomment{Panos: I am not sure we are
  qualified to make judgements on the intentions of the Copyright
  Law. In any case, as we claim later this has not been tested in a
  Court.}intended by the Copyright
...
reported on Github\pdfcomment{Panos: we need a footnote with a URL here},
...
templates derived from a grammar is capped\pdfcomment{Panos: How is the number capped? Who controls this?}.
\begin{figure}[t]
...
you can easily document it and share it. {\bf This immediately takes
  ...
  and compare it with an open-source DBMS} \pdfcomment{Panos: I do not
  understand what the boldface text is trying to express}. When a
...
others? Probably not.\pdfcomment{Panos: Why not? If not, how are other people
  going to use SQALPEL with their own DBMSs?} However, we can greatly
...
and host settings\pdfcomment{Panos: What are the private host
  settings?}. Otherwise, all relevant information is available in
...
commercial product: \emph{ You must obtain Microsoft's\pdfcomment{We
    should probably anonymize this reference} prior written approval
...
\pdfcomment{Panos: I propose we change the bold text in the two
  paragraphs above with the following: ``We believe that these
  restrictions are fundamentally incompatible with the essence of the
  scientific process. This is a discussion that needs to take place as
  a matter of some urgency in scientific, legal and commercial
  fora. One of the goals of this paper is to open such a
  discussion.''}
...
{\sc SQALPEL}\pdfcomment{Panos: Maybe explain a bit the client-server
  architecture of SQALPEL} is built as a web-based software platform
...
unless legal interaction is required.\pdfcomment{Panos: exposed to
  whom? What legal interaction do you mean here?}
...
syntactic\pdfcomment{Panos: Is the addition of the word "syntactic"
  correct in this context?} components in the query. Hovering over a
...
A principle\pdfcomment{Panos: I guess here you mean principal
  component analysis, but I am not sure we are performing the
  statistical procedure of that name (see
  https://en.wikipedia.org/wiki/Principal\_component\_analysis). Maybe
  "Identification of the dominant components" is more accurate?}
...
dominant term in Q1\pdfcomment{What is Q1? TPC-H if so, we should say
  so explicitly, or even better we should write the whole query out.}
...
Q1\pdfcomment{Panos: Again here either we should specifically say
  TPC-H scale factor 1, query 1, or write out the full query as an
  equation so that it can be referenced.}  runs about a factor 8
...
\begin{figure}[t!]
  \pdfcomment{Panos: The figures are really difficult to read and make
    sense of. If we have the option, maybe we should send an appendix
    with higher definition screenshots.}
  \centering
