...
% use this when using \usepackage[final]{pdfcomment}
%% \newcommand{\highlight}[1]{#1}
% use this when using \usepackage{pdfcomment}
...
\pdfcomment{Stefan:
Are the experiments conducted publically as the text says,
or shouls rather the reporting and sharing be public?
}
...
\highlight{Otherwise},
\pdfcomment{Stefan:
"Otherwise", i.e., if/when a performance project is not initiated and owned by someone?
Or if/when no contributors are invited to run the experiments and share results??
}
...
\pdfcomment{Stefan:
Do this mean the implicite "on the one hand" are the paragraphs above,
and they talk about public project, without mentioning so explicitely??
}
...
% with the first 2 figures (Fig.2: fig:sqalpel & Fig.3: fig:querypool2) below,
% - use 'width=\textwidth' for more readable Figures 2 & 3,
%   at the expense of references extending beyond page 6
% - use 'width=.807\textwidth' to fit/finish references on page 6,
%   at the expense of less readble Figures 2 & 3
